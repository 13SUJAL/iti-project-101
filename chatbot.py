...
try:
    ...
    print("Error: chatbot.txt file not found. Please ensure the file exists in the current directory.")
    exit(1)
...
def response(user_response):
    ...
    # Check if the user input has an additional response first
    ...
    try:
        ...
        if req_tfidf == 0:
            robo_response = "I am sorry, I don't understand. Could you please rephrase that?"
        else:
            robo_response = sent_tokens[idx]
    except Exception as e:
        robo_response = "I encountered an error processing your request. Could you try asking something else?"
...
while flag:
    try:
        user_response = input("You: ").strip()  # Add strip() to remove leading/trailing whitespace
        if not user_response:  # Check for empty input
            print("Julie: Please say something!")
            continue
        ...
        print("\nJulie: Goodbye! Feel free to come back if you have more questions.")
        break
    except Exception as e:
        print(f"Julie: Sorry, there was an error: {e}")
